...
export const checkForClutchPerformer = (matches: DbMatch[], player: DbPlayer) : boolean => {
    ...
    const matchesInvolvingPlayerAsWinner = matches.filter((match) => {return match.winning_player_id == player.id});
    const matchesThatWereWonBy2PointsOrLess = matchesInvolvingPlayerAsWinner.filter((match) => {return match.winning_player_score - match.losing_player_score <= 2})
...
export const checkForHeartBreaker = (matches: DbMatch[], player: DbPlayer) : boolean => {
    ...
    const matchesInvolvingPlayerAsLoser = matches.filter((match) => {return match.losing_player_id == player.id});
    const matchesThatWereLostBy2PointsOrLess = matchesInvolvingPlayerAsLoser.filter((match) => {return match.winning_player_score - match.losing_player_score <= 2})
...
export const checkForWinsInARow = (matches: DbMatch[], player: DbPlayer, count: number) : boolean => {
    ...
    const matchesInvolvingPlayer = matches.filter((match) => {return match.winning_player_id == player.id || match.losing_player_id == player.id});
...
export const checkForFatality = (matches: DbMatch[], player: DbPlayer) : boolean => {
    // Filter only to be the matches where the winner is this player
    const matchesInvolvingPlayerAsWinner = matches.filter((match) => {return match.winning_player_id == player.id});
    const matchesWonWithoutLosingAPoint = matchesInvolvingPlayerAsWinner.filter((match) => {return match.losing_player_score == 0});
...
export const checkForHelpless = (matches: DbMatch[], player: DbPlayer) : boolean => {
    // Filter only to be the matches where the loser is this player
    const matchesInvolvingPlayerAsLoser = matches.filter((match) => {return match.losing_player_id == player.id});
    const matchesLostWithoutEarningAPoint = matchesInvolvingPlayerAsLoser.filter((match) => {return match.losing_player_score == 0});
...
export const checkForSuperior = (matches: DbMatch[], player: DbPlayer) : boolean => {
    // Filter only to be the matches where the winner is this player
    const matchesInvolvingPlayerAsWinner = matches.filter((match) => {return match.winning_player_id == player.id});
    const matchesInvolving1300ELORank = matchesInvolvingPlayerAsWinner.filter((match) => {return match.winner_new_elo >= 1300});
...
export const checkForCellarDweller = (matches: DbMatch[], player: DbPlayer) : boolean => {
    // Filter only to be the matches where the loser is this player
    const matchesInvolvingPlayerAsLoser = matches.filter((match) => {return match.losing_player_id == player.id});
    const matchesInvolving1100ELORank = matchesInvolvingPlayerAsLoser.filter((match) => {return match.loser_new_elo >= 1100});
...
export const checkForALittleEmbarrassing = (matches: DbMatch[], player: DbPlayer) : boolean => {
    // Filter only to be the matches where the loser is this player
    const matchesInvolvingPlayerAsLoser = matches.filter((match) => {return match.losing_player_id == player.id});
    const matchesInvolvingALossToRelativeScrubOpponent = matchesInvolvingPlayerAsLoser.filter((match) => {return match.losing_player_original_elo - match.winning_player_original_elo >= 200});
...
export const checkForShouldHavePutMoneyOnIt = (matches: DbMatch[], player: DbPlayer) : boolean => {
    // Filter only to be the matches where the winner is this player
    const matchesInvolvingPlayerAsWinner = matches.filter((match) => {return match.winning_player_id == player.id});
    const matchesInvolvingLowOddsWin = matchesInvolvingPlayerAsWinner.filter((match) => {return match.losing_player_original_elo - match.winning_player_original_elo >= 200});

    return matchesInvolvingLowOddsWin.length > 0;
}

export const checkForTriggersAfterAMatch = (winningPlayer: DbPlayer, losingPlayer: DbPlayer, badges: DbBadge[], matches: DbMatch[], onError: (errorMsg: string)=>void) : void => {
    const decorateAchievementForUpload = (badgeDesc : BadgeDesc, earnedPlayer: DbPlayer, involvedPlayer: DbPlayer) : DbBadge => {
        return {
            ...badgeDesc,
            id: uuidv4(),
    ...
    const winnerBadgeKeys = badges.filter((badge: DbBadge) => { return badge.player_id === winningPlayer.id}).flatMap((badge) => badge.key);
    const loserBadgeKeys = badges.filter((badge: DbBadge) => { return badge.player_id === losingPlayer.id}).flatMap((badge) => badge.key);

    if (!winnerBadgeKeys.includes(Achievement.FATALITY)) {
        if (checkForFatality(matches, winningPlayer)) {
            QuickHitAPI.addBadge(decorateAchievementForUpload(FATALITY_BADGE, winningPlayer, losingPlayer), ()=>{return}, onError);
        }
    }

    if(!winnerBadgeKeys.includes(Achievement.CLUTCH_PERFORMER)) {
        if (checkForClutchPerformer(matches, winningPlayer)) {
            QuickHitAPI.addBadge(decorateAchievementForUpload(CLUTCH_PERFORMER_BADGE, winningPlayer, losingPlayer), ()=>{return}, onError);
        }
    }

    if(!winnerBadgeKeys.includes(Achievement.ON_A_ROLL)) {
        if (checkForWinsInARow(matches, winningPlayer, 3)) {
            QuickHitAPI.addBadge(decorateAchievementForUpload(ON_A_ROLL_BADGE, winningPlayer, losingPlayer), ()=>{return}, onError);
        }
    }

    if(!winnerBadgeKeys.includes(Achievement.UNSTOPPABLE)) {
        if(checkForWinsInARow(matches, winningPlayer, 5)) {
            QuickHitAPI.addBadge(decorateAchievementForUpload(UNSTOPPABLE_BADGE, winningPlayer, losingPlayer), ()=>{return}, onError);
        }
    }

    if(!winnerBadgeKeys.includes(Achievement.ASCENDED)) {
        if(checkForWinsInARow(matches, winningPlayer, 10)) {
            QuickHitAPI.addBadge(decorateAchievementForUpload(ASCENDED_BADGE, winningPlayer, losingPlayer), ()=>{return}, onError);
        }
    }

    if(!winnerBadgeKeys.includes(Achievement.SUPERIOR)) {
        if(checkForSuperior(matches, winningPlayer)) {
            QuickHitAPI.addBadge(decorateAchievementForUpload(SUPERIOR_BADGE, winningPlayer, losingPlayer), ()=>{return}, onError);
        }
    }
    ...
    if(!winnerBadgeKeys.includes(Achievement.SHOULD_HAVE_PUT_MONEY_ON_IT)) {
        if(checkForShouldHavePutMoneyOnIt(matches, winningPlayer)) {
            QuickHitAPI.addBadge(decorateAchievementForUpload(SHOULD_HAVE_PUT_MONEY_ON_IT_BADGE, winningPlayer, losingPlayer), ()=>{return}, onError);
        }
    }

    if(!loserBadgeKeys.includes(Achievement.A_LITTLE_EMBARRASSING)) {
        if(checkForALittleEmbarrassing(matches, losingPlayer)) {
            QuickHitAPI.addBadge(decorateAchievementForUpload(A_LITTLE_EMBARRASSING_BADGE, winningPlayer, losingPlayer), ()=>{return}, onError);
        }
    }

    if(!loserBadgeKeys.includes(Achievement.HELPLESS)) {
        if(checkForHelpless(matches, losingPlayer)){
            QuickHitAPI.addBadge(decorateAchievementForUpload(HELPLESS_BADGE, winningPlayer, losingPlayer), ()=>{return}, onError);
        }
    }

    if(!loserBadgeKeys.includes(Achievement.HEART_BREAKER)) {
        if(checkForHeartBreaker(matches, losingPlayer)) {
            QuickHitAPI.addBadge(decorateAchievementForUpload(HEART_BREAKER_BADGE, winningPlayer, losingPlayer), ()=>{return}, onError);
        }
    }

    if(!loserBadgeKeys.includes(Achievement.CELLAR_DWELLAR)) {
        if(checkForCellarDweller(matches, losingPlayer)) {
            QuickHitAPI.addBadge(decorateAchievementForUpload(CELLAR_DWELLAR_BADGE, winningPlayer, losingPlayer), ()=>{return}, onError);
        }
    }
}
